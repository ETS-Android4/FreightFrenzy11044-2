...
public class PurePursuitTracker {
	...
	public static double feedbackMultiplier = 0.01;
...
//	}
	...
	public PurePursuitTracker(DrivetrainTank drivetrainTank, Localizer localizer, LinearOpMode opMode, Lift lift, RobotIMU imu) {
...
//	 * @param heading current angle of robot
	...
	public DrivePower update(double currtime) {
		...
		for (int i = closestPointIndex + 1; i < robotPath.size(); i++) {
			...
			if (i == robotPath.size() - 1) //modified  && (robotPath.size() - 1) - starti < 20
				onLastSegment = true;
